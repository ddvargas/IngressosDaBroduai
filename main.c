...
int get_max_lotacao(int id_evento);
...
//        sem_init(&(eventos[i].full), 0, 0);
        ...
        eventos[i].lugares = (STATUS *) malloc(sizeof(STATUS) * max_lot);

        for (int j = 0; j < max_lot; j++) {
...
int solicitar_ingresso(int id_evento) {
    ...
    if (id_evento >= 0) {
        ...
        for (int i = 0; i < max_lotacao_evento; i++) {
...
void relatorio() {
    ...
    for (int i = 0; i < num_eventos; i++) {
        int vendidos = 0;
        max_lotation = get_max_lotacao(i);
        printf("Evento: %s\nLugares: ", eventos[i].nome);
        fprintf(trace, "Evento: %s\nLugares: ", eventos[i].nome);
        for (int j = 0; j < max_lotation; j++) {
        ...
        printf("\n(%d/%d) vendidos - R$%.2f\n",
               vendidos, max_lotation, (vendidos * eventos[i].valor_ingresso));
        fprintf(trace, "\n(%d/%d) vendidos - R$%.2f\n",
                vendidos, max_lotation, (vendidos * eventos[i].valor_ingresso));
    }
}
...
/**
 * Retorna o numero máximo de lotação do evento
 * @param id_evento id do evento que se quer consultar
 * @return numero máximo de lotação ou -1 se algum problema acontecer
 */
int get_max_lotacao(int id_evento) {
    int lotacao = -1;
    if (id_evento >= 0) {
        sem_wait(&eventos[id_evento].mutex_max);
        lotacao = eventos[id_evento].max_lotacao;
        sem_post(&eventos[id_evento].mutex_max);
    }
    return lotacao;
}